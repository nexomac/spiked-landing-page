<script>
	...
	import { MessageSquare, Sparkles, Brain, Target, FileText, CheckCircle2, TrendingUp } from 'lucide-svelte';
...
>
	...
	<div class="animated-border rounded-2xl overflow-hidden depth-shadow">
		...
		<div class="bg-gradient-to-br from-zinc-950 to-zinc-900 p-1 relative overflow-hidden">
			...
			<div class="grid grid-cols-[1fr_1.2fr] relative z-10" style="height: {height};">
						...
						<div class="flex gap-1.5 flex-wrap mb-3">
							{#each questionTypes as questionType}
								<button 
									...
									class="px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all {selectedQuestion === questionType ? 'bg-red-500/20 text-red-400 ring-1 ring-red-500/50' : 'glass text-zinc-500 hover:text-zinc-300'}"
								>
									{questionType === 'customization' ? 'Custom' : questionType.charAt(0).toUpperCase() + questionType.slice(1)}
								</button>
							{/each}
						</div>
