<script>
	...
	import { Moon, Sun, Monitor, Cpu, Globe, Rocket, Shield, Users, Zap, BookOpen, Newspaper, LifeBuoy, ChevronDown } from 'lucide-svelte';
    ...
    const navData = {
        platform: {
            label: 'Platform',
            links: [
                { title: 'Core Agent OS', desc: 'The engine powering autonomous growth.', icon: Cpu, href: '/features' },
                { title: 'Agent Simulator', desc: 'Predict and visualize outcomes.', icon: Globe, href: '/features/simulator' },
                { title: 'Spiked API', desc: 'Seamlessly integrate with your CRM.', icon: Zap, href: '/features/crm' }
            ]
        },
        solutions: {
            label: 'Solutions',
            links: [
                { title: 'Smart Follow-up', desc: 'Never miss a lead with automated reach.', icon: Rocket, href: '/features/followup' },
                { title: 'Revenue Ops', desc: 'Sentiment-driven funnel optimization.', icon: Shield, href: '/features/sentiment' },
                { title: 'Customer Success', desc: 'AI Notetaker for perfect retention.', icon: Users, href: '/features/notetaker' }
            ]
        },
        resources: {
            label: 'Resources',
            links: [
                { title: 'The Bulletin', desc: 'Weekly AI strategies and trends.', icon: Newspaper, href: '/blog' },
                { title: 'Dev Hub', desc: 'Technical tools for builders.', icon: BookOpen, href: '/features/dev-tools' },
                { title: 'Enterprise AI', desc: 'Custom goals for market leaders.', icon: LifeBuoy, href: '/features/custom-goals' }
            ]
        }
    };
...
<!-- Navbar -->
<nav class={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${navShellClass}`} onmouseleave={handleMouseLeave}>
	<div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-5">
		<div class="flex items-center justify-between">
			...
			<div class="hidden md:flex items-center gap-1">
                ...
                {#each Object.entries(navData) as [id, menu]}
                    <div class="relative group" role="presentation" onmouseenter={() => handleMouseEnter(id)}>
                        <button 
                        ...
                        >
                            {menu.label}
                            <ChevronDown class={`w-3.5 h-3.5 transition-transform duration-500 ${activeMenu === id ? 'rotate-180 text-red-500' : ''}`} />
                            <span class={`absolute bottom-0 left-4 right-4 h-0.5 bg-red-500 transition-all duration-300 ${activeMenu === id ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'}`}></span>
                        </button>
                    </div>
                {/each}

				<a href="/pricing" class={`px-4 py-2 text-sm font-bold transition-all duration-300 hover:scale-105 relative group ${$page.url.pathname === '/pricing' ? 'text-red-500' : (($page.url.pathname.startsWith('/blog') || $page.url.pathname.startsWith('/newsletter')) ? 'text-white' : (isLight ? 'text-slate-600 hover:text-red-600' : 'text-zinc-400 hover:text-red-500'))}`}>
					Pricing
            ...
            {#if activeMenu}
                ...
                >
                    ...
                    >
                        <div class="grid grid-cols-3 divide-x divide-zinc-800/10 dark:divide-white/5">
                            {#each Object.entries(navData) as [id, menu]}
                                <div 
                                    role="presentation"
                                ...
                                >
                                    <!-- Dynamic accent bar -->
                                    <div class={`absolute top-0 left-8 right-8 h-1 rounded-b-full transition-all duration-500 ${activeMenu === id ? 'bg-red-600 shadow-[0_2px_10px_rgba(220,38,38,0.5)]' : 'bg-transparent'}`}></div>
                                    
                                    <!-- Dynamic background highlight -->
                                    <div class={`absolute inset-0 transition-opacity duration-500 ${activeMenu === id ? (isLight ? 'bg-red-50/60 border-x border-red-100/30' : 'bg-red-950/10 border-x border-red-900/20') : 'opacity-0'}`}></div>
                                    
                                    <div class="relative z-10">
                                        <h3 class={`text-[11px] font-black uppercase tracking-[0.2em] mb-8 transition-all duration-500 ${activeMenu === id ? 'text-red-600 translate-x-1' : (isLight ? 'text-slate-400' : 'text-zinc-500')}`}>
                                            {menu.label}
                                        </h3>
                                        <div class="space-y-7">
                                            {#each menu.links as link}
                                                ...
                                                >
                                                    <div class={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 group-hover/item:scale-110 group-hover/item:shadow-xl ${isLight ? 'bg-white border border-slate-100 text-slate-400 group-hover/item:text-red-600 group-hover/item:border-red-100' : 'bg-zinc-900 border border-white/5 text-zinc-500 group-hover/item:text-red-500 group-hover/item:border-red-900/50'} ${activeMenu === id ? 'ring-1 ring-red-500/30 shadow-md bg-gradient-to-br from-red-600/5 to-transparent' : ''}`}>
                                                        <link.icon size={20} strokeWidth={activeMenu === id ? 2.5 : 2} />
                                                    </div>
                                                    <div class="flex-1 min-w-0">
                                                        <p class={`text-sm font-bold mb-1 transition-colors ${isLight ? 'text-slate-800 group-hover/item:text-red-600' : 'text-white group-hover/item:text-red-500'} ${activeMenu === id ? (isLight ? 'text-red-800' : 'text-white') : ''}`}>{link.title}</p>
                                                        <p class={`text-xs leading-relaxed transition-colors ${isLight ? 'text-slate-500' : 'text-zinc-500 group-hover/item:text-zinc-400'} ${activeMenu === id ? (isLight ? 'text-slate-600' : 'text-zinc-300') : ''}`}>{link.desc}</p>
                                                    </div>
                                                </a>
                                            {/each}
                                        </div>
                        ...
                        <!-- Panel Footer -->
                        <div class={`px-8 py-4 flex items-center justify-between border-t ${isLight ? 'bg-slate-50 border-slate-100' : 'bg-zinc-900/50 border-white/5'}`}>
                            <p class={`text-[10px] font-bold uppercase tracking-widest ${isLight ? 'text-slate-400' : 'text-zinc-500'}`}>Ready to scale your revenue?</p>
                            <a href="/contact-sales" class="text-[10px] font-black uppercase tracking-widest text-red-500 hover:text-red-600 transition-colors flex items-center gap-2">
                                Request a Demo →
                            </a>
                        </div>
                    </div>
		...
		<!-- Mobile Menu -->
		{#if mobileMenuOpen}
			<div class={`md:hidden mt-4 pb-6 space-y-6 pt-6 animate-slide-down rounded-2xl border ${isLight ? 'border-red-100 bg-white/95 shadow-lg shadow-red-100/40' : 'border-red-900/30 bg-black/80 backdrop-blur-xl'} px-4 max-h-[80vh] overflow-y-auto`}>
				{#each Object.entries(navData) as [id, menu]}
                    <div class="space-y-3">
                        <h3 class={`text-[10px] font-black uppercase tracking-widest px-2 ${isLight ? 'text-slate-400' : 'text-zinc-500'}`}>{menu.label}</h3>
                        <div class="grid grid-cols-1 gap-1">
                            {#each menu.links as link}
                                ...
                                >
                                    <div class={`w-8 h-8 rounded-lg flex items-center justify-center ${isLight ? 'bg-slate-100' : 'bg-zinc-800'}`}>
                                        <link.icon size={16} />
                                    </div>
                                    <span class="text-sm font-bold">{link.title}</span>
                                </a>
                            {/each}
                        </div>
                    </div>
                ...
                <div class="space-y-3 pt-2 border-t border-zinc-800/10 dark:border-white/5">
                    <a href="/pricing" class={`block px-3 py-2 text-sm font-bold rounded-lg transition-all ${isLight ? 'hover:bg-red-50 text-slate-700 hover:text-red-600' : 'hover:bg-red-950/20 text-zinc-300 hover:text-red-500'}`} onclick={() => mobileMenuOpen = false}>Pricing</a>
                    <a href="/blog" class={`block px-3 py-2 text-sm font-bold rounded-lg transition-all ${isLight ? 'hover:bg-red-50 text-slate-700 hover:text-red-600' : 'hover:bg-red-950/20 text-zinc-300 hover:text-red-500'}`} onclick={() => mobileMenuOpen = false}>The Bulletin</a>
                </div>
				...
				<div class="pt-4 space-y-3">
					...
					<button class={`block w-full text-center px-4 py-3 text-sm font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${isLight ? 'text-slate-700 border border-slate-200 hover:text-red-600 hover:border-red-200 bg-white' : 'text-white hover:text-red-500 border border-zinc-800'}`}>
						Log In
					</button>
					<button 
						onclick={onboardingStore.start}
