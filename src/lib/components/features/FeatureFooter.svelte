...
<footer class="bg-zinc-950 border-t border-zinc-800">
	<div class="max-w-7xl mx-auto px-6 py-16">
				...
				{#each otherFeatures.slice(0, 4) as feature}
					...
					>
						<div class="flex items-center justify-between">
							<span
								class="text-sm font-semibold text-zinc-300 group-hover:text-white transition-colors"
								>{feature.title}</span
							>
							<ArrowRight
								class="w-4 h-4 text-zinc-600 group-hover:text-red-500 group-hover:translate-x-1 transition-all"
							/>
						</div>
					</a>
				{/each}
		...
		<!-- CTA Section -->
		<div
			class="bg-gradient-to-r from-zinc-900 to-zinc-950 rounded-2xl p-8 md:p-12 border border-zinc-800 text-center"
		>
			<h3 class="text-3xl md:text-4xl font-bold text-white mb-4">
				Ready to Transform Your Sales Process?
			</h3>
			<p class="text-lg text-zinc-400 mb-8 max-w-2xl mx-auto">
				Join thousands of sales professionals using SpikedAI to close more deals
				and work smarter.
			</p>
		...
		<!-- Footer Links -->
		<div
			class="mt-12 pt-8 border-t border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-4"
		>
			<div class="flex items-center gap-2">
				<img src="/icon white.png" alt="SpikedAI" class="h-6 w-6 rounded" />
