<script>
	...
	import { 
		...
		Sparkles, Layers, BarChart3, RefreshCw, Download, Share, CheckCircle2, Plus, Menu
	} from 'lucide-svelte';
...
>
	...
	<div class="animated-border rounded-2xl overflow-hidden depth-shadow h-full flex flex-col bg-card">
		{#if showAppChrome}
			<!-- App Chrome -->
			<div class="bg-zinc-900/95 border-b border-white/5 px-4 py-3 flex items-center justify-between backdrop-blur-xl shrink-0">
				<div class="flex items-center gap-3">
					...
					<div class="flex items-center gap-2 px-3 py-1 glass rounded-lg hover:bg-zinc-800/70 transition-colors">
						...
						<span class="text-sm font-black tracking-tight text-white hidden sm:inline">
				...
				<div class="flex items-center gap-2">
                    <!-- Mobile Menu Toggle -->
                    <button class="md:hidden p-2 text-zinc-400 hover:text-white" onclick={() => isSidebarOpen = !isSidebarOpen}>
                        <Menu class="w-4 h-4" />
                    </button>
                    
					<button class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-lg transition-all shadow-lg shadow-red-500/20">
						<Download class="w-3.5 h-3.5" />
						Save PDF
					</button>
					<button class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-all shadow-lg shadow-blue-500/20">
		...
		<!-- Main Content Area -->
		<div class="bg-gradient-to-br from-zinc-900 to-black p-1 relative overflow-hidden flex-1 min-h-0">
			<!-- Animated gradient overlay -->
			<div class="absolute inset-0 bg-gradient-to-br from-red-500/5 via-transparent to-blue-500/5 opacity-50 animate-pulse pointer-events-none"></div>
			
			<div class="grid grid-cols-1 md:grid-cols-[260px_1fr] h-full relative z-10">
				
				<!-- Left Sidebar (Collapsible on Mobile) -->
				<div class="{isSidebarOpen ? 'absolute inset-0 z-50 bg-zinc-900/95' : 'hidden'} md:block md:relative border-r border-white/5 p-4 space-y-6 backdrop-blur-sm overflow-y-auto custom-scrollbar">
                    
                    {#if isSidebarOpen}
                         <button class="absolute top-4 right-4 text-zinc-400" onclick={() => isSidebarOpen = false}>Close</button>
                    {/if}
					...
					<div class="space-y-1">
						<button 
							...
							class="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 text-sm group {activeTab === 'templates' ? 'glass text-white shadow-lg' : 'text-zinc-400 hover:text-white hover:bg-white/5'}"
						>
						...
						<button 
							...
							class="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 text-sm group {activeTab === 'custom' ? 'glass text-white shadow-lg' : 'text-zinc-400 hover:text-white hover:bg-white/5'}"
						>
					...
					<!-- Sessions -->
					<div>
						<p class="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2 px-1">Recent</p>
						<div class="space-y-1">
							...
							<button class="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-all duration-300 group text-zinc-500 hover:text-zinc-300 hover:bg-white/5">
				...
				<!-- Main Panel -->
				<div class="relative overflow-hidden flex flex-col h-full bg-zinc-900/50">
					<!-- Background ambient glow -->
					<div class="absolute inset-0 bg-gradient-to-br from-red-500/5 via-transparent to-transparent opacity-30 pointer-events-none"></div>
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
    					{#if activeTab === 'templates'}
    						...
    						<div class="space-y-4">
    							<div class="mb-4">
                                    <h3 class="text-sm font-semibold text-white mb-1">Templates</h3>
    								<p class="text-xs text-zinc-500">Select a template to view generated output</p>
    							</div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
    								{#each displayTemplates as template}
    									...
    									<button
    										...
    										class="text-left p-3 rounded-xl transition-all duration-300 group hover:scale-[1.02] border border-white/5 {selectedTemplate?.id === template.id ? 'bg-white/10 border-red-500/30' : 'bg-transparent hover:bg-white/5'}"
    									>
    										<div class="flex items-center gap-3">
    											<div class="w-8 h-8 rounded-lg bg-gradient-to-br {template.color} flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 transition-transform">
    												<IconComponent class="w-4 h-4 text-white" />
    											</div>
    											<div class="min-w-0">
    												<h4 class="font-semibold text-white text-sm group-hover:text-red-400 transition-colors truncate">{template.title}</h4>
                                                </div>
    										</div>
    									</button>
    								{/each}
        							...
        							<div class="space-y-4 pt-4 border-t border-white/5">
        								...
        								<div class="glass rounded-xl p-5 border border-white/10">
        									<div class="flex items-center gap-2 mb-3">
        										...
        										<h4 class="font-semibold text-white text-sm">{selectedTemplate.title} Framework</h4>
        									</div>
