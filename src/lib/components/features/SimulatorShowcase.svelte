<script>
	...
	let visibleDialogue = $derived(
		displayDialogue.slice(0, currentDialogueIndex + 1),
	);
	let currentCoaching = $derived(
		displayCoaching[selectedScenario] || displayCoaching.discovery,
	);
...
>
	...
	></div>
	<div
		class="absolute -bottom-8 -left-8 w-40 h-40 bg-purple-500/10 rounded-full blur-3xl float-slow"
	></div>
	...
	<div class="animated-border rounded-2xl overflow-hidden depth-shadow">
		{#if showAppChrome}
			...
			>
				<div class="flex items-center gap-3">
					...
					>
						...
						<div class="flex items-center gap-2">
							...
							<span class="text-[11px] text-zinc-400 uppercase tracking-wide"
								>Simulator</span
							>
						</div>
					</div>
				...
				<div class="flex items-center gap-2">
					{#if simulationActive}
						<div
							class="flex items-center gap-1.5 px-3 py-1.5 bg-green-950/40 border border-green-900/50 rounded-full"
						>
							<div
					...
					{:else}
						<button
							class="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 text-xs font-semibold rounded-lg transition-all"
						>
		...
		>
			...
			<div
				class="absolute inset-0 bg-gradient-to-br from-red-500/5 via-transparent to-purple-500/5 opacity-50 animate-pulse"
			></div>
					...
					<div class="p-6 h-full overflow-y-auto scrollbar-thin">
						...
						<div class="grid grid-cols-2 gap-3 mb-6">
							{#each displayScenarios as scenario}
								...
								{@const gradientClass =
									colorGradients[scenario.color] || colorGradients.blue}
								<button
									onclick={() => selectScenario(scenario.id)}
									class="group flex flex-col items-start gap-3 p-4 rounded-xl border transition-all text-left {selectedScenario ===
									scenario.id
										? 'glass border-red-500/30 shadow-lg shadow-red-500/10'
										: 'glass-subtle hover:glass'}"
								>
									<div
										class="w-10 h-10 rounded-lg bg-gradient-to-br {gradientClass} flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-110 transition-transform"
									>
									...
									<div class="flex-1">
										<div class="flex items-center gap-2 mb-1">
											...
											{#if selectedScenario === scenario.id}
												<CheckCircle
													class="w-3.5 h-3.5 text-red-500 animate-pulse"
												/>
											{/if}
										</div>
										...
										<div class="flex items-center gap-2">
											<span
												class="px-2 py-0.5 glass rounded text-[10px] font-semibold text-zinc-400"
											>
												{scenario.difficulty}
											</span>
											<span
					...
					<div class="h-full p-1">
						<div class="grid grid-cols-[1fr_1.3fr] h-full">
							...
							<div class="border-r border-border/50 glass-subtle flex flex-col">
								...
								<div class="px-3 py-2.5 border-b border-border/50 glass">
									<div class="flex items-center gap-2.5">
										<div
											class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white text-[10px] font-bold shadow-lg"
										>
											JC
										</div>
										<div class="flex-1">
										...
										<div
											class="flex items-center gap-1 px-2 py-0.5 glass border border-green-500/30 rounded-full"
										>
											<Circle
												class="w-1.5 h-1.5 fill-green-500 text-green-500"
											/>
											<span class="text-[9px] font-semibold text-green-400"
												>ENGAGED</span
											>
										</div>
									</div>
								...
								>
									{#each visibleDialogue as dialogue}
										...
										>
											<div
												class="flex gap-2 {dialogue.isAI
													? ''
													: 'flex-row-reverse'}"
											>
												{#if dialogue.isAI}
													<div
														class="w-6 h-6 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0 shadow-md"
													>
														{dialogue.initials}
													</div>
												{:else}
													<div
														class="w-6 h-6 rounded-full bg-gradient-to-br from-green-600 to-green-700 flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0 shadow-md"
													>
														{dialogue.initials}
													</div>
												{/if}
												...
												<div class="flex-1 max-w-[85%]">
													<div
														class="flex items-center gap-1.5 mb-1 {dialogue.isAI
															? ''
															: 'flex-row-reverse'}"
													>
														<span
														...
														{#if dialogue.sentiment}
															<span
																class="px-1.5 py-0.5 glass rounded text-[8px] text-zinc-400"
															>
																{dialogue.sentiment}
															</span>
														{/if}
													...
													<div
														class="p-2.5 rounded-lg {dialogue.isAI
															? 'glass border border-border/50'
															: 'glass border border-green-500/30 shadow-sm shadow-green-500/10'}"
													>
														<p
															class="text-[11px] text-zinc-300 leading-relaxed"
														>
															{dialogue.text}
														</p>
															...
															<div class="mt-2 pt-2 border-t border-border/50">
																...
																>
																	...
																	<span class="text-xs font-bold text-green-500"
																		>{dialogue.score}/100</span
																	>
																</div>
																<div
																	class="flex items-start gap-1 text-[9px] text-muted-foreground"
																>
																	<CheckCircle
																		class="w-2.5 h-2.5 text-green-500 flex-shrink-0 mt-0.5"
																	/>
																...
																>
																	<div class="flex items-start gap-1.5">
																		...
																		{:else if point.type === "critical"}
																			<AlertCircle
																				class="w-2.5 h-2.5 text-red-500 flex-shrink-0 mt-0.5"
																			/>
																		{:else if point.type === "buying_signal"}
																			<Sparkles
																				class="w-2.5 h-2.5 text-blue-500 flex-shrink-0 mt-0.5"
																			/>
																		{:else}
																			<Lightbulb
																				class="w-2.5 h-2.5 text-orange-500 flex-shrink-0 mt-0.5"
																			/>
																		{/if}
																		<div class="flex-1">
																			<p
																				class="text-[9px] font-semibold text-white mb-0.5"
																			>
																				{point.text}
																			</p>
									...
									{#if !isPaused && currentDialogueIndex < displayDialogue.length - 1}
										...
										<div class="flex gap-2 items-center">
											<div
												class="w-6 h-6 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white text-[9px] font-bold shadow-md"
											>
												JC
											</div>
											<div
												class="px-3 py-2 glass border border-border/50 rounded-lg flex gap-1"
											>
												<div
													class="w-1.5 h-1.5 bg-zinc-600 rounded-full typing-dot"
												></div>
												<div
													class="w-1.5 h-1.5 bg-zinc-600 rounded-full typing-dot"
												></div>
							...
							<div class="flex flex-col glass-subtle">
								<!-- Coaching Header -->
								<div
									class="px-3 py-2.5 border-b border-border/50 flex items-center justify-between glass"
								>
								...
								{#if showCoaching}
									...
									>
										...
										<div
											class="glass border border-red-500/30 rounded-xl p-3 shadow-sm shadow-red-500/10"
										>
											<div class="flex items-center justify-between mb-2">
												<div class="flex items-center gap-2">
													<div
														class="w-7 h-7 rounded-lg bg-gradient-to-br from-red-600 to-red-700 flex items-center justify-center shadow-lg"
													>
														<Award class="w-3.5 h-3.5 text-foreground" />
													</div>
													<div>
														<h6 class="text-[10px] font-bold text-foreground">
															Overall Performance
														</h6>
														<p class="text-[8px] text-red-400">
															Real-time analysis
														</p>
													</div>
												</div>
											...
											<div class="flex items-center gap-2">
												<div
													class="flex-1 h-1.5 bg-secondary/50 rounded-full overflow-hidden"
												>
													<div
														class="h-full bg-gradient-to-r from-red-600 to-red-500 rounded-full transition-all"
														style="width: {currentCoaching.overallScore}%"
													></div>
												</div>
												<span class="text-[10px] font-semibold text-green-500"
													>{currentCoaching.sentiment}</span
												>
											</div>
										</div>
										...
										<div class="glass border border-border/50 rounded-xl p-3">
											<h6
												class="text-[10px] font-bold text-white mb-2 flex items-center gap-1.5"
											>
											...
											<div class="space-y-1.5">
												{#each currentCoaching.keyMetrics as metric}
													<div
														class="flex items-center justify-between p-2 glass-subtle rounded-lg border border-border/30"
													>
														<div class="flex-1">
															<div class="flex items-center gap-1.5 mb-0.5">
																<span
																	class="text-[9px] font-semibold text-foreground"
																	>{metric.label}</span
																>
														...
														<div
															class="text-xs font-bold {metric.status === 'good'
																? 'text-green-500'
																: 'text-orange-500'}"
														>
															{metric.value}
										...
										<!-- Strengths -->
										<div
											class="glass border border-green-500/30 rounded-xl p-3"
										>
											<h6
												class="text-[10px] font-bold text-white mb-2 flex items-center gap-1.5"
											>
											...
											<ul class="space-y-1">
												{#each currentCoaching.strengths as strength}
													<li
														class="flex items-start gap-1.5 text-[10px] text-zinc-300"
													>
														<CheckCircle
															class="w-2.5 h-2.5 text-green-500 flex-shrink-0 mt-0.5"
														/>
										...
										<div
											class="glass border border-orange-500/30 rounded-xl p-3"
										>
											<h6
												class="text-[10px] font-bold text-white mb-2 flex items-center gap-1.5"
											>
