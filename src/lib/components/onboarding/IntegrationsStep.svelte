	...
	<div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
		{#each integrations as integration}
			<button
				...
				class="group relative bg-zinc-950 border-2 {selectedIntegrations.includes(
					integration.id,
				)
					? 'border-red-600'
					: 'border-zinc-900'} hover:border-red-900/50 rounded-xl p-5 text-center transition-all duration-300 transform hover:-translate-y-1 {integration.id ===
				'none'
			...
			>
				<div class="flex flex-col items-center gap-3">
					<div
						class="w-16 h-16 rounded-lg border border-zinc-800 flex items-center justify-center {integration.logoBg ??
							'bg-zinc-900'}"
					>
					...
					<div>
						<h3
							class="text-base font-bold text-white group-hover:text-red-500 transition-colors"
						>
							{integration.label}
						</h3>
					...
					{#if selectedIntegrations.includes(integration.id)}
						<div class="absolute top-2 right-2">
							<div
								class="w-5 h-5 bg-red-600 rounded-full flex items-center justify-center"
							>
								<svg
		...
		>
			<span class="flex items-center justify-center gap-2">
				{selectedIntegrations.length > 0
					? `Continue with ${selectedIntegrations.length} selected`
					: "Continue"}
				<svg
