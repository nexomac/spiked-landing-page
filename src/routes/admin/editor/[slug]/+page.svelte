	...
	function addBlock(type) {
		...
		switch (type) {
			...
			case "richtext":
				...
				break;
			case "quote":
				newBlock.data = { text: "", author: "", authorUrl: "" };
				break;
			case "statistics":
				newBlock.data = { value: "", label: "" };
				break;
			case "image":
				newBlock.data = { url: "", caption: "" };
				break;
			case "code":
				newBlock.data = { code: "", language: "javascript" };
				break;
			case "link":
				newBlock.data = { value: "" };
				break;
...
{#if isSaving}
	<div
		class="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center"
	>
...
<div
	class="min-h-screen bg-black text-white pt-24 pb-24 px-4 sm:px-6 lg:px-8 font-sans"
>
	<div class="max-w-4xl mx-auto">
		...
		<header class="mb-10">
			...
			<div
				class="flex justify-between items-center mb-6 pb-4 border-b border-gray-800"
			>
				<a
					href="/admin"
					class="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white transition group"
				>
					<ArrowLeft
			...
			<div class="space-y-6">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div>
						<label
							class="block text-xs font-mono text-gray-500 mb-1"
							for="blog-slug">Slug</label
						>
						<input
							type="text"
							id="blog-slug"
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
					<div>
						<label
							class="block text-xs font-mono text-gray-500 mb-1"
							for="blog-date">Published Date</label
						>
						<input
							type="datetime-local"
							id="blog-date"
						...
						<div class="mt-6">
							<label
								class="block text-xs font-mono text-gray-500 mb-1"
								for="blog-cover">Cover Image</label
							>
							<input
								type="text"
								id="blog-cover"
							...
							<div class="flex items-center gap-4">
								<label
									class="cursor-pointer bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded text-xs text-indigo-300 transition flex items-center gap-2"
								>
									<span>Upload</span>
									<input
										type="file"
							...
							{#if blog.coverImage}
								<div
									class="mt-2 w-full h-32 bg-black/40 rounded-lg overflow-hidden border border-gray-800"
								>
									<img
										src={blog.coverImage}
										alt="Cover"
					...
					<div>
						<div class="block text-xs font-mono text-gray-500 mb-1">
							Newsletters
						</div>
						<div
							class="bg-gray-900/50 border border-gray-800 rounded-lg p-3 max-h-[250px] overflow-y-auto space-y-1"
						>
							{#each newsletters as newsletter}
								<label
									class="flex items-start gap-3 p-2 hover:bg-white/5 rounded cursor-pointer transition select-none"
								>
									<input
										type="checkbox"
										value={newsletter._id}
										checked={blog.newsletterIds
											? blog.newsletterIds.includes(newsletter._id)
											: false}
										onchange={(e) =>
											toggleNewsletter(newsletter._id, e.target.checked)}
										class="mt-0.5 rounded border-gray-700 bg-gray-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-gray-900"
									/>
									<div>
										<span class="block text-sm text-gray-300 font-medium"
											>{newsletter.title}</span
										>
										{#if newsletter.description}
											<span class="block text-xs text-gray-600 line-clamp-1"
							...
							{#if newsletters.length === 0}
								<p class="text-xs text-gray-600 italic p-2">
		...
		<div class="space-y-6 min-h-[500px]">
			{#each blocks as block, index (block.id)}
				<div
					class="group relative bg-gray-900/50 border border-gray-800 rounded-xl p-6 transition hover:border-gray-700"
				>
					<!-- Block Controls -->
					<div
						class="absolute right-4 top-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition duration-200"
					>
						<button
							onclick={() => moveBlock(index, "up")}
							disabled={index === 0}
							class="p-1.5 text-gray-500 hover:text-white hover:bg-gray-800 rounded disabled:opacity-30"
						>
						...
						<button
							...
							class="p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded"
						>
					...
					<!-- Block Labels -->
					<div
						class="absolute -top-3 left-4 px-2 bg-gray-950 border border-gray-800 text-[10px] uppercase font-bold tracking-wider text-gray-500 rounded-full"
					>
						{block.type}
					</div>
					...
					<div class="mt-2">
						{#if block.type === "header"}
							<div class="flex items-center gap-2 mb-2">
								<select
									...
									onchange={(e) =>
										updateBlockData(index, "level", parseInt(e.target.value))}
						...
						{:else if block.type === "quote"}
							<div
								class="pl-6 border-l-4 border-red-600 bg-gray-900/40 p-6 rounded-r-xl space-y-4"
							>
								<div class="space-y-1">
									<label
										class="block text-[10px] font-mono text-gray-500 uppercase tracking-widest"
										for="quote-body-{index}">Quote Body</label
									>
									<textarea
										id="quote-body-{index}"
										value={block.data.text}
										oninput={(e) =>
											updateBlockData(index, "text", e.target.value)}
								...
								<div
									class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-800"
								>
									<div class="space-y-1">
										<label
											class="block text-[10px] font-mono text-gray-500 uppercase tracking-widest"
											for="quote-author-{index}">Attributed To</label
										>
										<input
											...
											oninput={(e) =>
												updateBlockData(index, "author", e.target.value)}
									...
									<div class="space-y-1">
										<label
											class="block text-[10px] font-mono text-gray-500 uppercase tracking-widest"
											for="quote-link-{index}">Link (Optional)</label
										>
										<div class="flex items-center gap-2">
											...
											<input
												...
												oninput={(e) =>
													updateBlockData(index, "authorUrl", e.target.value)}
						...
						{:else if block.type === "statistics"}
							<div
								class="flex items-center gap-6 p-4 bg-gray-800/30 rounded-lg"
							>
								<div class="flex-1">
									<label class="text-xs text-gray-500" for="stats-value-{index}"
										>Value</label
									>
									<input
										type="text"
										...
										oninput={(e) =>
											updateBlockData(index, "value", e.target.value)}
										class="w-full bg-transparent text-4xl font-bold text-indigo-400 border-none focus:ring-0 p-0"
										placeholder="50%"
									/>
								</div>
								<div class="flex-[2]">
									<label class="text-xs text-gray-500" for="stats-label-{index}"
										>Label</label
									>
									<input
										type="text"
										...
										oninput={(e) =>
											updateBlockData(index, "label", e.target.value)}
						...
						{:else if block.type === "image"}
							<div class="space-y-4">
								<div>
									<label
										class="block text-xs font-mono text-gray-500 mb-1"
										for="img-url-{index}">Image URL</label
									>
									<input
										type="text"
										id="img-url-{index}"
										...
										oninput={(e) =>
											updateBlockData(index, "url", e.target.value)}
										class="w-full bg-gray-900 border border-gray-800 rounded px-3 py-2 text-sm text-gray-300"
										placeholder="https://..."
									/>
									<p class="text-[10px] text-gray-600 mt-1">
										Paste a direct image link (e.g. from existing DB upload or
										external)
									</p>
									<label
										class="inline-block mt-2 cursor-pointer bg-gray-800 hover:bg-gray-700 px-3 py-1 rounded text-xs text-indigo-400 transition"
									>
										Upload from PC
										<input
											type="file"
								...
								{#if block.data.url}
									<div
										class="relative w-full h-48 bg-black/20 rounded overflow-hidden flex items-center justify-center border border-gray-800 border-dashed"
									>
										<img
											src={block.data.url}
											alt="Preview"
								...
								<div>
									<label
										class="block text-xs font-mono text-gray-500 mb-1"
										for="img-cap-{index}">Caption</label
									>
									<input
										type="text"
										id="img-cap-{index}"
										...
										oninput={(e) =>
											updateBlockData(index, "caption", e.target.value)}
						...
						{:else if block.type === "link"}
							<div class="space-y-4">
								<div>
									<label
										class="block text-xs font-mono text-gray-500 mb-1"
										for="link-url-{index}">Link / YouTube URL</label
									>
									<input
										type="text"
										id="link-url-{index}"
								...
								{#if getYoutubeId(block.data.value)}
									<div
										class="w-full aspect-video rounded-xl overflow-hidden bg-black border border-gray-800 shadow-lg"
									>
										<iframe
											width="100%"
											height="100%"
		...
		<div class="mt-12 border-t border-gray-800 pt-8">
			...
			<div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
				<button
					...
					class="flex flex-col items-center justify-center gap-2 p-4 bg-gray-900 border border-gray-800 rounded-xl hover:border-gray-600 hover:bg-gray-800 transition"
				>
				...
				<button
					...
					class="flex flex-col items-center justify-center gap-2 p-4 bg-gray-900 border border-gray-800 rounded-xl hover:border-gray-600 hover:bg-gray-800 transition"
				>
					<LinkIcon class="w-5 h-5 text-yellow-400" />
					<span class="text-xs font-medium">Link/YouTube</span>
				</button>
			</div>
		</div>
	</div>
