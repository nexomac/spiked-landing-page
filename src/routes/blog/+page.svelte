<script>
	...
	const featuredPosts = $derived(sortedPosts.slice(0, 3));
	const restPosts = $derived(sortedPosts.slice(3));
...
>
	<div class="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-12">
		...
		{#snippet postRow(post, index)}
			<article
				class="py-16 md:py-24 border-b {$themeStore === 'dark'
					? 'border-zinc-800'
					: 'border-zinc-200'} last:border-b-0"
			>
				<a href="/blog/{post.slug}" class="group block">
					<div class="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
						<div class={index % 2 === 1 ? "lg:order-2" : "lg:order-1"}>
							{#if index === 0}
								<p
									class="text-[10px] uppercase tracking-[0.3em] text-red-500 font-semibold mb-6"
								>
									Spotlight
								</p>
							{/if}
							<h2
								class="text-3xl sm:text-4xl lg:text-5xl font-semibold leading-tight group-hover:text-red-500 transition-colors"
							>
								{post.title}
							</h2>
							<p
								class="mt-6 text-base sm:text-lg leading-relaxed {$themeStore ===
								'dark'
									? 'text-zinc-400'
									: 'text-zinc-600'}"
							>
							...
							<ul class="mt-8 space-y-4">
								{#each (post.points && post.points.length ? post.points : defaultPoints).slice(0, 3) as point}
									<li class="flex items-start gap-3">
										<span
											class="mt-2 h-1.5 w-1.5 rounded-full bg-red-500 shrink-0"
										></span>
										<span
											class="text-sm sm:text-base leading-relaxed {$themeStore ===
											'dark'
												? 'text-zinc-300'
												: 'text-zinc-600'}"
										>
							...
							<div
								class="mt-10 flex items-center gap-4 text-[11px] uppercase tracking-[0.25em] {$themeStore ===
								'dark'
									? 'text-zinc-500'
									: 'text-zinc-400'}"
							>
						...
						<div class={index % 2 === 1 ? "lg:order-1" : "lg:order-2"}>
							<div
								class="aspect-[16/10] sm:aspect-[4/3] rounded-3xl overflow-hidden border transition-all duration-700 group-hover:rounded-[2rem] {$themeStore ===
								'dark'
			...
			{:else}
				...
				<section class="my-16">
					...
					<div class="mt-14 pb-10">
						<div
							class="flex items-center justify-between pb-8 border-b border-zinc-800"
						>
							<h2 class="text-xl sm:text-2xl font-semibold">
								Newsletter Briefings
							</h2>
							<span
								class="text-xs uppercase tracking-[0.2em] {$themeStore ===
								'dark'
									? 'text-zinc-500'
									: 'text-zinc-400'}">Updated Weekly</span
							>
						</div>
