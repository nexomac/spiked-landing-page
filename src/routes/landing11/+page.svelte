...
<div class="page-shell min-h-screen transition-colors duration-500 font-sans selection:bg-red-500/30 overflow-x-hidden {$themeStore === 'dark' ? 'bg-[#030712] text-zinc-100' : 'bg-white text-zinc-900'}">
	<main class="relative z-10 w-full overflow-hidden max-w-[1900px] mx-auto bg-transparent">
		<!-- HERO: theme-aware dark/light, CEO-led, quote + Avi photo -->
		<section
			data-section="hero"
			class="hero-section relative w-full min-h-[85vh] flex flex-col lg:flex-row items-center justify-between overflow-hidden transition-colors duration-500
				{$themeStore === 'dark'
					? 'hero-dark'
					: 'hero-light'}"
			style="padding: clamp(2rem, 5vw, 4rem); padding-top: clamp(6rem, 14vh, 10rem); padding-bottom: clamp(4rem, 10vh, 6rem);"
		>
			<!-- Background gradient: theme-aware -->
			<div
				class="absolute inset-0 pointer-events-none transition-opacity duration-500 {$themeStore === 'dark' ? 'hero-gradient-dark' : 'hero-gradient-light'}"
				aria-hidden="true"
			></div>

			<!-- Left: Quote + CEO messaging -->
			<div class="relative z-10 w-full lg:w-[55%] max-w-3xl flex flex-col justify-center order-2 lg:order-1">
				<!-- Affiliations -->
				<div
					class="flex flex-wrap items-center gap-3 mb-6 transition-colors duration-300
						{$themeStore === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}"
				>
					<span
						class="px-3 py-1.5 rounded-lg text-xs font-medium tracking-wide transition-colors duration-300
							{$themeStore === 'dark'
								? 'border border-zinc-600/70 bg-zinc-800/40 text-zinc-400'
								: 'border border-zinc-300 bg-white/80 text-zinc-600 shadow-sm'}"
					>Google for Startups</span>
					<span
						class="px-3 py-1.5 rounded-lg text-xs font-medium tracking-wide transition-colors duration-300
							{$themeStore === 'dark'
								? 'border border-zinc-600/70 bg-zinc-800/40 text-zinc-400'
								: 'border border-zinc-300 bg-white/80 text-zinc-600 shadow-sm'}"
					>NVIDIA Inception</span>
				</div>

				<span
					class="text-red-500 text-5xl sm:text-6xl font-serif leading-none block mb-4 transition-colors duration-300"
					aria-hidden="true"
				>"</span>

				<h1
					class="font-bold leading-[1.15] tracking-tight mb-6 transition-colors duration-300
				...
				<p
					class="font-medium leading-relaxed mb-10 transition-colors duration-300
						{$themeStore === 'dark' ? 'text-zinc-400' : 'text-zinc-600'}"
					style="font-size: clamp(1.05rem, 1.8vw, 1.35rem);"
				>
					Built for leaders who make decisions in live customer moments, turning conversations into conversions.
				</p>
				...
				<div class="flex flex-wrap items-center gap-6 mb-10">
					<div class="flex items-center gap-4">
						<div
							class="w-14 h-14 rounded-full overflow-hidden flex-shrink-0 ring-2 transition-all duration-300
								{$themeStore === 'dark'
									? 'border-2 border-red-500/50 ring-red-500/20'
									: 'border-2 border-red-500/40 ring-red-500/15 shadow-md'}"
						>
							<img
								src="/Photos/Avi Sahi.jpeg"
								alt="Avi Sahi"
								class="hero-avatar-img w-full h-full object-cover object-top"
							/>
						</div>
						<div>
							<div
								class="font-bold text-lg transition-colors duration-300
									{$themeStore === 'dark' ? 'text-white' : 'text-zinc-900'}"
							>Avi Sahi</div>
							<div class="text-red-500 font-semibold tracking-widest text-[10px] uppercase">Co-Founder & CEO, SpikedAI</div>
						</div>
					</div>
					<a
						href="/contact-sales"
						class="hero-cta group inline-flex items-center gap-2 px-6 py-3.5 rounded-full font-semibold text-white bg-red-600 hover:bg-red-500 border-2 border-red-600 hover:border-red-500 shadow-lg shadow-red-900/20 hover:shadow-red-500/30 transition-all duration-300 hover:scale-[1.02]"
					>
						Book a Demo
			...
			<!-- Right: Avi photo -->
			<div class="relative z-10 w-full lg:w-[45%] flex justify-center lg:justify-end order-1 lg:order-2 mb-10 lg:mb-0">
				<div
					class="hero-photo-wrap relative w-full max-w-lg aspect-[3/4] lg:aspect-auto lg:min-h-[70vh] lg:max-h-[85vh] transition-all duration-500
						{$themeStore === 'dark'
							? 'shadow-2xl shadow-black/50'
							: 'shadow-2xl shadow-zinc-400/25 ring-1 ring-zinc-200/80'}"
				>
					<img
						src="/Photos/Avi Sahi.jpeg"
						alt="Avi Sahi, Co-Founder & CEO of SpikedAI"
						class="hero-photo w-full h-full object-cover object-top"
					/>
					<div
						class="absolute inset-0 pointer-events-none transition-opacity duration-300 {$themeStore === 'dark' ? 'hero-photo-overlay-dark' : 'hero-photo-overlay-light'}"
						aria-hidden="true"
					></div>
				</div>
			</div>
		...
		<!-- Transition from hero to content (theme-aware) -->
		<div
			class="h-8 sm:h-12 w-full transition-colors duration-500
				{$themeStore === 'dark'
					? 'bg-gradient-to-b from-[#0a0a0a] to-[#030712]'
					: 'bg-gradient-to-b from-[#fafafa] to-white'}"
			aria-hidden="true"
		></div>

		<div class="px-[clamp(1.5rem,4vw,4rem)] transition-colors duration-500">
				...
				<!-- 2. Vision line (quote + Avi in hero above) -->
				<div
					class="rounded-2xl py-8 px-6 sm:px-10 text-center {$themeStore === 'dark' ? 'bg-zinc-900/50 border border-zinc-800' : 'bg-zinc-100 border border-zinc-200'}"
				>
					<p
						class="font-semibold max-w-3xl mx-auto {$themeStore === 'dark' ? 'text-zinc-300' : 'text-zinc-700'}"
						style="font-size: clamp(1.1rem, 1.5vw, 1.4rem); line-height: 1.5;"
					>
						We bring clarity into the chaos—supporting every kind of seller to perform at their highest potential.
					</p>
				</div>
				...
				<!-- 4. Impact pillars (text-only, modern; no dated graphics) -->
				<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
					{#each impactItems as item}
						<div
							class="rounded-2xl p-8 transition-all duration-300 flex flex-col
								{$themeStore === 'dark'
								? 'bg-zinc-900/50 border border-zinc-800'
								: 'bg-zinc-100 border border-zinc-200'}"
						>
							<span class="text-2xl font-black text-red-500 mb-4 block">{item.id}</span>
							<h4
								class="text-xl font-bold mb-3 {$themeStore === 'dark'
									? 'text-white'
									: 'text-zinc-900'}"
							>
								{item.title}
							...
							<p
								class="font-medium text-sm leading-relaxed {$themeStore === 'dark'
									? 'text-zinc-400'
									: 'text-zinc-600'}"
							>
								{item.description}
...
<style>
	...
	.hero-section.hero-light {
		background: linear-gradient(135deg, #fafafa 0%, #f4f4f5 50%, #ffffff 100%);
	}
	.hero-gradient-dark {
		background: linear-gradient(105deg, #0a0a0a 0%, #0f0f0f 45%, #141414 100%);
	}
	.hero-gradient-light {
		background: linear-gradient(105deg, transparent 0%, rgba(255,255,255,0.4) 45%, rgba(255,255,255,0.6) 100%);
	}
	.hero-photo-wrap {
